...
- TC 的 `vel_psd` 必须与 TC 的量测门限和零偏先验联合验证。`8-23/MECH-13` 当前通过值为 `0.0003`，并配套 `maxinno=30`、固定零杆臂和窄加计零偏先验；不能把 LC 的 `0.5` 或旧 TC 值直接复制到当前配置。
...
`8-23` 的诊断表明，速度误差锯齿主要来自 `P_vel` 人工托底与 `P[vel, ba]` 相关性被削弱：加计零偏反馈曾为 `e-8` 级，导致 `ba` 在早期错误收敛后冻结。将 TC `vel_psd` 降至 `0.0003`、把 `maxinno` 从 `1` 放宽到 `30`、关闭在线杆臂估计，并给定窄的加计零偏先验后，`k_ba` 恢复约 `2300` 倍，速度反馈锯齿明显下降。
