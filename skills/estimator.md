...
- TC 的 `vel_psd` 必须与 TC 的量测门限和零偏先验联合验证。`8-23/MECH-13` 的 `maxinno=30` 是历史实验条件；当前相对定位应使用相位 `maxinno=5` 与伪距 `maxcode=30`，再与固定零杆臂和窄加计零偏先验联合评估。不能把 LC 的 `0.5` 或旧 TC 值直接复制到当前配置。
