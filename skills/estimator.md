...
> **当前状态索引（2026-09-15）**：当前估计器为单滤波 EKF，`LcEstimator` 与 `TcEstimator` 共用 INS 误差状态和 Joseph 更新；TC 的 GNSS 参数块、过程噪声运行点和反馈诊断已纳入实现。INS 参数已拆 `ins`/`ins_tc`/`ins_lc` 三段，`pos_psd`/`vel_psd` 从机制禁止用于 TC（见 [项目当前状态](项目当前状态.md) 第 4.4 节）。不要使用本文历史参数扫描替代 [项目当前状态](项目当前状态.md) 中的 TC/LC 边界。
...
- TC 的 `pos_psd` 必须配置为 `0`；自 2026-09-15 起它是 LC-only 参数（`INS_LC_ONLY_KEYS`），写在共用 `ins` 段且 `ins.enabled='tc'` 时加载器直接报错，历史 `pos_psd=1e-8` 注入量已隔离到休眠的 `ins_lc` 段。否则位置协方差被人为持续放大，并经 `P[pos, vel]` 交叉项传递为约 `1 m/s` 的更新后速度常值偏差。
- TC 的 `vel_psd` 同为 LC-only 机制禁止，必须写在 `ins_lc` 段；`8-23/MECH-13` 的 `maxinno=30` 是历史实验条件；当前相对定位应使用相位 `maxinno=5` 与伪距 `maxcode=30`，再与固定零杆臂和窄加计零偏先验联合评估。不能把 LC 的 `0.5` 或旧 TC 值直接复制到当前配置。
- LC 不得照搬 TC 的 `pos_psd=0`。高精度 LC 位置更新可能令 `P_pos` 坍缩，导致速度状态失去合理过程不确定性；LC 应保留配置文件中经过验证的非零值（写在 `ins_lc` 段生效）。
...
### 7.5 `8-23` 加计零偏可观测性结论（2026-08-24）

`8-23` 的诊断表明，速度误差锯齿主要来自 `P_vel` 人工托底与 `P[vel, ba]` 相关性被削弱：加计零偏反馈曾为 `e-8` 级，导致 `ba` 在早期错误收敛后冻结。当时将 TC `vel_psd` 降至 `0.0003`、把 `maxinno` 从 `1` 放宽到 `30`、关闭在线杆臂估计，并给定窄的加计零偏先验后，`k_ba` 恢复约 `2300` 倍。该结论只说明零偏可观测性改善；Data19 后续验证表明 `maxinno=30` 会放过未初始化相位行，当前应保持相位 `5`、伪距 `30` 的分离门限。

这不是单纯的 `ba` 初值修复。实验还显示：

1. 仅将 `vel_psd` 置零会因旧创新门限造成量测饥饿和 RTK→SPP 反复降级；
2. 在线杆臂估计在当前数据上可收敛到非物理值并与姿态耦合；
3. 加计零偏标定值需要窄先验，否则滤波会重新吸收系统力误差；
4. 低 `vel_psd` 方案必须在目标窗口、全段和最差机动窗口同时验收。

当前结论是“误差状态与协方差通道已显著改善”，不是“所有数据集均可复用同一组零偏”。更换数据集或 IMU 后，必须重新估计标定初值、先验和创新门限。
...
### 7.6 `9-14` 零偏初值与过程模型修复（2026-09-15）

`8-11` 的零偏可观测性结论（7.5 节）只解决了 `vel_psd` 与门限的联合调参，未触及两个更根本的机械编排级根因（见 `issue/9-14机械编排发散.md` 与 [机械编排.md 第 15.7 节](机械编排.md#157-9-14-机械编排发散根因零偏可估计性--过程模型修复2026-09-15)）：

- **加计零偏初始标准差偏小 102 倍**：`acce_bias_std_si` 应为 `0.05 m/s²`（=5.11 mg），历史误填 `4.890e-4 m/s²`（0.05 mg），导致 `P_ba` 过早收敛冻结、机械编排每秒漂移约 `0.5 m`。修复显式 `acce_bias_std_si: [0.05, 0.05, 0.05]`。
- **零偏过程模型 τ 阻尼**：GREAT 零偏为纯随机游走（`_tauG/_tauA=0`，`F` 零偏块 `eb-eb`/`db-db`）；gipylib 旧默认 `corr_time=36 s` 引入 `-I/τ` 阻尼，使 `P_ba` 收敛到 `Q·τ/2` 稳态后不再增长，滤波器拒绝继续修正零偏。修复显式 `gyro_bias_corr_time_s: .inf` / `acce_bias_corr_time_s: .inf`。

二者修复后，GREAT campus01 对照窗口 gipylib `E/N/U = 0.4500/0.2155/0.2267 m`，优于 GREAT `0.8627/0.7334/0.4594 m`。排查量测间速度异常时，应**先核对 `acce_bias_std_si` 量级（m/s² 而非 mg）与零偏 `corr_time` 是否为 `.inf`**，再查协方差公式与观测门限。
