"""对比 spptc.rslt (LLH) 与 truth.csv (ECEF) 的位置/速度/姿态精度。

spptc.rslt: LLH (lat, lon, h) + ECEF 速度 + roll/pitch/yaw, 100Hz
...
def load_rslt(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            # 按最近整数秒匹配 (IMU 100Hz 有 ~4ms 偏移: 357453.996 ≈ 357454.0)
            data[(week, int(round(sow)))] = rec
    return data
...
def main():
    ours = load_rslt("data/output/spptc.rslt")
    truth = load_truth("data/truth.csv")
    print(f"spptc.rslt 历元: {len(ours)}")
